(function () {
    class RestApiWidget extends HTMLElement {
        constructor() {
            super();
            this._shadowRoot = this.attachShadow({ mode: "open" });
            this._data = null; // Initialize data
            this._fetchData(); // Fetch data on initialization
        }

        async _fetchData() {
            try {
                const response = await fetch("https://prods.cfapps.eu10-004.hana.ondemand.com/predict?product=MR311651"); // Replace with your API URL
                if (!response.ok) throw new Error('Network response was not ok');
                this._data = await response.json(); // Assume JSON response
                this._fireDataFetched(this._data);
            } catch (error) {
                console.error("Fetch error: ", error);
            }
        }

        _fireDataFetched(data) {
    ...
    customElements.define("com-bcx-widget-restapi", RestApiWidget);
})();
